...
class Vehicle {
    ...
    func returnFuelType () -> String {
        fuelType
    }
    
    func changeGear (to gear: Int) -> Int {
        currentGear = gear
        return currentGear
    }
    
    func speedup (to speed: Double) -> Double {
        self.currentSpeed = speed
        return currentSpeed
    }
    
    func applyBrake (speedDecrase: Double) -> Double {
        self.currentSpeed = speedDecrase
        return currentSpeed
    }
...
let vehicle = Vehicle(fuel: "--")
let electricVehicle = electric(fuel: "Electricity")
let dieselVehicle = diesel(fuel: "Diesel")
let petrolVehicle = petrol(fuel: "Petrol")

// vehicleChangeGearTo2.changeGear(to: 2)
print("\(electricVehicle.returnFuelType()):: New gear: \(electricVehicle.changeGear(to: 2))")
print("\(electricVehicle.returnFuelType()):: New gear: \(electricVehicle.changeGear(to: 5))")
print("\(electricVehicle.returnFuelType()):: New speed: \(electricVehicle.speedup(to: 50.0))")
print("\(electricVehicle.returnFuelType()):: Speed decreace to: \(electricVehicle.applyBrake(speedDecrase: 25))\n")

print("\(dieselVehicle.returnFuelType()):: New speed: \(dieselVehicle.speedup(to: 100.0))")
print("\(dieselVehicle.returnFuelType()):: Speed decreace to: \(dieselVehicle.applyBrake(speedDecrase: 10))")
print("\(dieselVehicle.returnFuelType()):: New gear: \(dieselVehicle.changeGear(to: 5))\n")

print("\(petrolVehicle.returnFuelType()):: New speed: \(petrolVehicle.speedup(to: 10))")
print("\(petrolVehicle.returnFuelType()):: Speed decreace to: \(petrolVehicle.applyBrake(speedDecrase: 0))")
print("\(petrolVehicle.returnFuelType()):: New gear: \(petrolVehicle.changeGear(to: 1))")
